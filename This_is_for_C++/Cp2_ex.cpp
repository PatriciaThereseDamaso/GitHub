/* 
CHANGELOG:
Integrated YesNoPrompt() on returnBook()
Removed variable char menumode entirely
Respective declarations on each function removed
Changed conditional statements inside YesNoPrompt
Further explanation on CODE ANALYSIS 
Removed borrowHistory() in function declaration
Removed char choice and std:: in void addBook()
Removed <limits>

   ERROR:
Found error in signing up as a user after adding a book and signing out as an admin, the return to menu exits the program immediately. [FIXED]
...
if (delete_display != 2) YesNoPrompt(prompt, origin);
}
