...
void adminMenu(), addBook(), deleteBook();
void userMenu(), borrowBook(), returnBook(), borrowHistory();
void displayBook(), searchBook();

void YesNoPrompt(string prompt, char menumode, char origin);
...
char menumode = 'B', origin = 'S';

YesNoPrompt(prompt, menumode, origin);
}
...
void signIN() {
    ...
    do {
        ...
        if (found) {
            remove("userProfiles.txt");
            rename("temp.txt", "userProfiles.txt");
        }
        else {
...
void adminMenu() {
    ...
    while (true) {
        ...
        // Check if the input is within the valid range
        if (valid_input && choice.length() == 1 && choice >= "1" && choice <="6") {
            input = stoi(choice);
            break;
        }
        else {
...
void addBook() {
    ...
    ofstream shelf("library_records.txt", std::ios::app);
    if (!shelf.is_open()) {
        std::cerr << " \x1b[31m Unable to open file for writing! \x1b[0m" <<std::endl;
        return;
    }
    ...
    bool findE = false;
    for (char c : temp_course) { //checks each letter on string for E (engineering)
        if (toupper(c) == 'E') {
            findE = true;
            break;
        }
    ...
    //gets 3rd and 4th digit of bookID (inventory ID)
    ...
    //gets 5th and 6th digit of bookID (year ID)
...
void displayBook() {
    ...
    if (shelf.is_open()) {
        ...
        // Inside the displayBook() function
...
void searchBook() {
    ...
    cout << "Search an Item \x1b[34m(BookID, Name, Author, Course, Publication Year, Status)\n\x1b[0m: ";
    cout.flush();
    if (cin.peek() == '\n')
        cin.ignore();
    ...
    while (getline(shelf, line)) {
        ...
        if (line.find(searchterm) != string::npos) {
            ...
            //Copied Angels function for splitting it into individual fields
            istringstream iss(line);
...
void borrowBook() {
    ...
    // Move file only if the book's status has been updated
    if (bookUpdated) {
...
bool exitloop = false;
do {
    cout << "\x1b[32m\n[1]\x1b[0m Return to Menu\x1b[32m\n[2]\x1b[0m Exit";
...
if (returnChoice == 1 && userMode == 'A') adminMenu();
    else if (returnChoice == 1 && userMode == 'U') userMenu();
...
string validCourse() {
    ...
    while (!isValid) {
        ...
        if (course == "BSCS" || course == "BSIT" || course == "BSMMA" || course == "BSCE"
            ...
            cout << "\x1b[31m\nInvalid course. Please follow the format and enter courses existing in COA.\x1b[0m" << std::endl;
        }
    }
    return course;
}
...
if (menumode == 'B' && origin == 'S') {
    ...
    // General Y/N prompt when trying to add, search, borrow, and display
    bool exitloop = false;
